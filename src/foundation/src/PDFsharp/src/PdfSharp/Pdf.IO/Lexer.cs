...
namespace PdfSharp.Pdf.IO
{
    ...
    class Lexer
    {
        ...
        public int DetermineStreamLength(SizeType start, int searchLength, SuppressExceptions? suppressObjectOrderExceptions = null)
        {
            ...
            var lastChar = idxEndStream > 0 ? rawString[idxEndStream - 1] : ' ';
            var secondLastChar = idxEndStream > 1 ? rawString[idxEndStream - 2] : ' ';
            if (secondLastChar == '\r' && lastChar == '\n')
                return idxEndStream - 2;
